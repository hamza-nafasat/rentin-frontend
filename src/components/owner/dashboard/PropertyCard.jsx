    ...
    <div className="flex flex-col gap-4 md:flex-row">
      ...
      <div className="flex basis-full flex-col justify-between md:basis-[65%]">
        <div className="flex justify-between gap-5">
          <h4 className="text-textPrimary text-sm font-medium md:text-base">{data?.name}</h4>
          <span
            className={`${
              data?.rental === 'Rent' ? 'bg-[#1D7FFF1A] text-[#0245A5]' : 'bg-[#E3545433] text-[#E35454]'
            } h-6 rounded-xs px-[6px] py-[3px] text-xs font-semibold`}
          >
            {data?.rental}
          </span>
        </div>
        <p className="text-textSecondary text-xs md:text-sm">123 Sunset Road, Phuket, Thailand</p>
        <div className="flex items-center justify-between">
          <p className="text-base font-semibold md:text-lg">
            ${data?.price}
            <span className="text-textSecondary text-xs">Per month</span>
          </p>
          <Link href="" className="rounded-xs bg-[#EEFFF2] p-[6px] text-sm font-semibold text-[#39DA4C]">
            Details
          </Link>
        </div>
      </div>
