...
const ShowMap = ({ properties = [] }) => {
  // Default center position (you can adjust this based on your needs)
  const centerMarkerPosition = [13.7563, 100.5018];

  // Filter properties that have valid latitude and longitude
  const propertiesWithLocation = properties.filter(property => property.latitude && property.longitude);
    ...
    >
      ...
      >
        ...
        {propertiesWithLocation.map(property => {
          ...
          const propertyImage = property?.images?.[0]?.url || 'https://via.placeholder.com/300x200';
          const rentPrice = property?.deals?.[0]?.rent || property?.contractRate?.rate || 0;
          const propertyStatus = property?.isBooked ? 'Rented' : 'Free';

          return (
            <Marker key={property._id} position={position} icon={buildingIcon}>
              <Popup>
                <div style={{ width: '200px' }}>
                  <img
                    src={propertyImage}
                  ...
                  <p style={{ margin: '4px 0', fontSize: '14px', color: '#666' }}>
                    {property.building} in {property.address}
                  </p>
                  <div
                    style={{
                      display: 'flex',
                  ...
                  >
                    <span style={{ fontWeight: '600', color: '#333' }}>${rentPrice}.00/month</span>
                    <span
                      style={{
                        background: propertyStatus === 'Free' ? '#ffe066' : '#ffcccc',
                        color: '#333',
                        padding: '2px 6px',
                        borderRadius: '4px',
                        fontSize: '12px',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    <span style={{ marginRight: '4px' }}>4.5</span>
                    <img src="/images/dashboard/star.png" alt="star" style={{ width: '16px', height: '16px' }} />
                  </div>
                </div>
              </Popup>
            </Marker>
