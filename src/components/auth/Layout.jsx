...
const Layout = ({ title, secondTitle, thirdTitle, children }) => {
  return (
    <section className="h-full w-full bg-[url('/images/default/auth-bg.png')] bg-cover bg-bottom bg-no-repeat lg:h-screen">
      <section className="scroll-0 xl:[80px] relative container mx-auto grid h-full grid-cols-1 gap-8 overflow-auto px-5 py-10 lg:grid-cols-2 lg:gap-[60px] lg:py-[10px]">
        {/* logo */}
        <Image
...
const ThirdTitle = ({ title }) => (
  <h6 className="mt-3 text-center text-sm font-medium text-white lg:text-left lg:text-lg">{title}</h6>
);
